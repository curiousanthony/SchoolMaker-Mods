...
function turnChaptersIntoCollapsible() {
	const frame = document.querySelector(
		'div[data-controller="product-view"] div#product-section-view-frame'
	)

	// CHECK: If frame doesn't have any descendant with class "section-separator", abort
	if (frame && !frame.querySelector(".section-separator")) {
		console.log(
			"turnChaptersIntoCollapsible: section-separator not found, aborting transformation"
		)
		return
	}

	const chapters = document.querySelectorAll(
		'div[data-controller="product-view"] div#product-section-view-frame > div'
	)
  ...
  `
    ...
    details.querySelector('[id^="products_section_"]').classList.remove(
        "!rounded-lg",
        "md:!rounded-xl"
      )

    // Add rounded corners on the bottom only
    details.querySelector('[id^="products_section_"]').classList.add(
        "!rounded-b-lg",
        "md:!rounded-b-xl"
      )
    */
